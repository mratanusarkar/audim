...
class PodcastTranscriber(BaseTranscriber):
    ...
    def export_subtitle(self, output_path: str) -> None:
        ...
        if self._processed_segments is None:
            raise ValueError("No processed audio available. Run `process_audio()` first.")
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            for i, segment in enumerate(self._processed_segments, 1):
                ...
                original_segment = next(
                    (s for s in self._segments_with_speakers if 
    ...
    def set_speakers(self, min_speakers: Optional[int] = None, max_speakers: Optional[int] = None) -> None:
    ...
    def set_speaker_names_display(self, show_speaker_names: bool, pattern: Optional[str] = None) -> None:
    ...
    def set_line_properties(self, max_length: int = 70, min_split_length: int = 50) -> None:
