...
class Subtitle:
    """
    Utility for replacing speaker placeholders with actual names in SRT files
        ...
        if isinstance(speakers, list):
            ...
            for key, name in speakers.items():
                ...
                # If the key already includes 'Speaker', use as is
                    ...
                    formatted_key = f"[{key}]" if not str(key).startswith('[') else str(key)
                    formatted_key = formatted_key if formatted_key.endswith(']') else f"{formatted_key}]"
